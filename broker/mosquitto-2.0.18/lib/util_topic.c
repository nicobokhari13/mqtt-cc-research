...
int mosquitto_sub_topic_check(const char *str)
{
	...
	while(str[0]){ // while there is a character in the first position
		// checks for wildcard characters
		if(str[0] == '+'){
			if((c != '\0' && c != '/') || (str[1] != '\0' && str[1] != '/')){
				return MOSQ_ERR_INVAL;
			}
		}else if(str[0] == '#'){// counting the hierarchies in the subscription string
			if((c != '\0' && c != '/')  || str[1] != '\0'){
				return MOSQ_ERR_INVAL;
			}
		}
#ifdef WITH_BROKER // counting the hierarchies in the subscription string
		else if(str[0] == '/'){ 
			hier_count++;
		}
#endif
		len++; // increasing the number of characters already traversed
		c = str[0]; //save the traversed character into c
		str = &str[1]; //shift the string down 1 character
	}
	if(len > 65535) return MOSQ_ERR_INVAL;
#ifdef WITH_BROKER
	if(hier_count > TOPIC_HIERARCHY_LIMIT) return MOSQ_ERR_INVAL;
