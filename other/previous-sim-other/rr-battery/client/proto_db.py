...
class Database:
    ...
    def __init__(self) -> None:
        self._db_path = "/home/devnico/repos/research/mqtt_cc_research/sqlite/mqttcc-rrBattery.db"
    ...
    def getCapableDevicesDescendingBattery(self, topicName):
        selectQuery = '''SELECT devices.deviceMac, battery, executions
                        FROM devices
                        LEFT JOIN publish
                        ON devices.deviceMac = publish.deviceMac
                        WHERE topic = ?
                        ORDER BY devices.battery DESC'''
        topicValue = (topicName,)
        return self.execute_query_with_retry(query=selectQuery, values=topicValue)
